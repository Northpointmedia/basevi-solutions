...
export async function POST(request: Request) {
  try {
    ...
    isExistingContact = contactResult.error?.statusCode === 409;
    ...
    const [confirmationAttempt, notificationAttempt] = await Promise.allSettled([
      resend.emails.send({
        from: fromEmail,
        to: [email],
      ...
      resend.emails.send({
        from: fromEmail,
        to: [businessEmail],
        subject: "Nueva suscripción a My Roots",
        html: `
          <div style="font-family:Arial,sans-serif;max-width:620px;margin:0 auto;color:#1f2a25">
            <h1 style="font-family:Georgia,serif;color:#173f33">Nueva suscripción a My Roots</h1>
    ...
    const confirmationSent =
      confirmationAttempt.status === "fulfilled" && !confirmationAttempt.value.error;
    const notificationSent =
      notificationAttempt.status === "fulfilled" && !notificationAttempt.value.error;

    if (!confirmationSent) {
      console.error("Newsletter confirmation email failed");
    }
    if (!notificationSent) {
      console.error("Newsletter business notification failed");
    }
